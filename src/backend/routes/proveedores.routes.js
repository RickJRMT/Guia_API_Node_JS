...
router.get('/', async (req, res) => {
    try {
        ...
        const proveedores = await crud.obtenerTodo(tabla);

        // Respuesta con el arreglo de proveedores en formato JSON
...
// Ruta para obtener una proveedor específica por su ID
router.get('/:id', async (req, res) => {
    try {
        // Utilizar el método obtenerUno con el ID recibido en la URL
        const proveedor = await crud.obtenerUno(tabla, idCampo, req.params.id);

        // Respuesta con datos de la proveedor en formato JSON
        res.json(proveedor);
    } catch (error) {
...
// Ruta para crear una nueva proveedor (registro nuevo en la base de datos)
router.post('/', async (req, res) => {
    try {
        // Utilizar el método crear con los datos enviados en el cuerpo del resquest
        const nuevaProveedor = await crud.crear(tabla, req.body);

        // Respuesta con el nuevo registro creado y código 201 (creado)
        res.status(201).json(nuevaProveedor);
    } catch (error) {
        // Manejar errores de servidor
        res.status(500).json({ error: error.message });
    }
...
// Ruta para actualizar una proveedor existente (por ID)
router.put('/:id', async (req, res) => {
    try {
        // Utilizar el método actualizar con el ID y los nuevos datos del cuerpo
        const proveedorActualizado = await crud.actualizar(tabla, idCampo, req.params.id, req.body);
...
// Ruta para eliminar una proveedor de la base de datos (por ID)}
